...
bool AchievementsView::Init()
{
	...
	textOutputDisplay = MakeTextOutputDisplay("ACHIEVEMENTS VIEW TEXT OUTPUT", 400, 150, 800, 400, [&]() {printf("Achievements View Text Output\n");textOutputDisplay->SetDisplayText(achievementsInitString);});
	textOutputDisplay->SetDisplayText(achievementsInitString);
	...
	offsetY += PADDING;
    buttons.emplace_back(MakeButton("ACHIEVEMENT TEST 0 SET", offsetX, offsetY, [&]() {printf("ACH_TEST_0 set\n");statsAndAchievements->SetAchievement("ACH_TEST_0");}));
	offsetY += PADDING;
    buttons.emplace_back(MakeButton("ACHIEVEMENT TEST 0 CLEAR", offsetX, offsetY, [&]() {printf("ACH_TEST_0 clear\n");statsAndAchievements->ClearAchievement("ACH_TEST_0");}));
	offsetY += PADDING;
